...
export function SectionHeader({ actions, className, description, title }: SectionHeaderProps) {
  return (
    <div className={cn("mb-4 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between", className)}>
    ...
    <div className={cn("grid gap-x-8 gap-y-4", columnClasses[columns], className)}>
      {items.map((item, index) => (
        <div key={`${item.label}-${index}`} className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-normal text-muted">{item.label}</p>
          <div className="mt-1.5 text-sm font-medium text-foreground">{item.content ?? item.value}</div>
        </div>
      ))}
    </div>
  );
