  ...
  const columns: DataTableColumn<CustomFieldRecord>[] = [
    ...
    { key: "usedInContacts", header: "Used In Contacts", cell: (row) => row.usedInContacts },
    {
      ...
      cell: (row) => (
        <TableActions
          actions={[
            { icon: Pencil, label: "Edit field", onClick: () => openEditDialog(row) },
            {
              icon: Power,
    ...
    <div className="flex w-full flex-col gap-4 pb-8">
      <SectionCard>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-base font-semibold text-foreground">Custom Attributes</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              {fieldCount} of {fieldLimit} custom fields used
            </p>
          </div>
      ...
      <SectionCard>
        <div className="mb-5">
          <h2 className="text-base font-semibold text-foreground">Field-Based Filtering</h2>
          <p className="mt-1 text-sm text-muted-foreground">Filter contacts by custom attributes.</p>
        </div>
        ...
        {appliedFilter ? (
          <div className="mt-5">
            {filterResults.length ? (
              ...
              <EmptyState title="No matching contacts" description="Try changing the field, condition, or value." />
            )}
          </div>
        ) : null}
      </SectionCard>
...
function CustomFieldDialog({
  ...
  return (
    <StandardDialog
      footerRight={<Button disabled={!fieldForm.fieldName.trim()} onClick={onSave}>Save Field</Button>}
