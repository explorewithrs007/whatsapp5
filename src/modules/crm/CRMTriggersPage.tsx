...
import { FlaskConical, Pencil, Power } from "lucide-react";
...
export function CRMTriggersPage() {
  ...
  const [activeTab, setActiveTab] = useState("keyword-automation");
  ...
  const columns: DataTableColumn<CRMTriggerRecord>[] = [
    ...
    { key: "lastUpdated", header: "Last Updated", cell: (row) => row.lastUpdated },
    {
      ...
      cell: (row) => (
        <TableActions
          actions={[
            { icon: Pencil, label: "Edit trigger", onClick: () => openEditTrigger(row) },
            { icon: FlaskConical, label: "Test trigger", onClick: () => testSingleTrigger(row) },
            {
              icon: Power,
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      <PageTabs
        defaultValue="keyword-automation"
        onValueChange={setActiveTab}
        tabs={[
          {
            value: "keyword-automation",
            label: "Keyword Automation",
            content: (
              <SectionCard>
                <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <h2 className="text-base font-semibold text-foreground">Keyword Automation</h2>
                    <p className="mt-1 text-sm text-muted-foreground">Manage keyword auto-responses.</p>
                  </div>
                ...
                {filteredTriggers.length ? (
                  ...
                  <EmptyState
                    ...
                    description="Try searching by trigger name, keyword, or response."
                  />
                )}
              </SectionCard>
            ),
          ...
          {
            value: "auto-reply-rules",
            label: "Auto-Reply Rules",
            content: (
              <SectionCard>
                <h2 className="text-base font-semibold text-foreground">Auto-Reply Rules</h2>
                <p className="mt-1 text-sm text-muted-foreground">Manage basic auto-reply rules.</p>
                <div className="mt-5 grid gap-3 lg:grid-cols-3">
                  {ruleRows.map((rule) => (
                    <div key={rule.name} className="rounded-xl border border-border bg-slate-50 p-4">
          ...
          },
          {
            ...
            content: (
              <SectionCard>
                <h2 className="text-base font-semibold text-foreground">Test Trigger</h2>
                <p className="mt-1 text-sm text-muted-foreground">Test keyword matching.</p>
                  ...
                  <div className="rounded-xl border border-border bg-slate-50 p-4">
                    ...
                    {testSubmitted ? (
                      testResult ? (
                        ...
                        <p className="text-sm text-muted-foreground">No trigger matched.</p>
                      )
                    ) : (
          ...
          },
          {
            ...
            content: (
              <SectionCard>
                <h2 className="text-base font-semibold text-foreground">Auto-Assignment</h2>
                <p className="mt-1 text-sm text-muted-foreground">
                  Assign chats to an available agent if no reply is sent within a timeout.
                </p>
                <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_220px_180px] lg:items-center">
                  <p className="text-sm text-muted-foreground">
...
function TriggerDialog({
  ...
  return (
    <StandardDialog
      ...
      footerRight={<Button disabled={!canSave} onClick={onSave}>Save Trigger</Button>}
...
function RuleDialog({
  ...
  return (
    <StandardDialog
      ...
      footerRight={<Button onClick={onSave}>Save Rule</Button>}
      onOpenChange={onOpenChange}
      open={open}
