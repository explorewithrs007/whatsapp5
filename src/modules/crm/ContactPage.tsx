  ...
  const contactColumns: DataTableColumn<ContactRecord>[] = [
    ...
    { key: "lastInteraction", header: "Last Interaction", cell: (row) => row.lastInteraction },
    {
      ...
      cell: (row) => (
        <TableActions
          actions={[
            { icon: Eye, label: "View profile", onClick: () => openViewProfile(row) },
            {
              icon: MessageSquareText,
              label: "View conversation history",
              onClick: () => openConversationHistory(row),
            },
            { icon: Pencil, label: "Edit contact", onClick: () => openEditContact(row) },
          ]}
        />
      ),
    },
    ...
    <div className="flex w-full flex-col gap-4 pb-8">
      <SectionCard>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-semibold text-foreground">Contact List</h2>
            <p className="mt-1 text-sm text-muted-foreground">Review WhatsApp contact records.</p>
          </div>
        ...
        {filteredContacts.length ? (
          ...
          <EmptyState title="No contacts found" description="Try searching by name, phone, email, or tag." />
        )}
      </SectionCard>
...
function ConversationHistoryDialog({
  ...
  const historyColumns: DataTableColumn<ConversationHistoryRecord>[] = [
    ...
    { key: "assignedAgent", header: "Assigned Agent", cell: (row) => row.assignedAgent },
    {
      ...
      cell: (row) => (
        <TableActions
          actions={[
            { icon: MessageSquareText, label: "View conversation", onClick: () => onConversationSelect(row) },
          ]}
        />
      ),
    },
  ...
  return (
    <StandardDialog onOpenChange={onOpenChange} open={Boolean(contact)} title="Conversation History">
      {contact && !conversation ? (
        <div className="space-y-5">
          ...
          {contactHistory.length ? (
            <DataTable columns={historyColumns} data={contactHistory} getRowId={(row) => row.id} />
          ) : (
            <EmptyState title="No conversation history" description="This contact has no WhatsApp conversations yet." />
          )}
        </div>
      ) : null}
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3 rounded-xl border border-border bg-slate-50 p-4">
            {messages.length ? (
              ...
              <p className="text-sm text-muted-foreground">No message timeline available.</p>
            )}
          </div>
        </div>
      ) : null}
  ...
  return (
    <div className="mt-2 grid gap-2 sm:grid-cols-2">
      {entries.map(([field, value]) => (
        <div className="rounded-xl border border-border bg-slate-50 p-3" key={field}>
