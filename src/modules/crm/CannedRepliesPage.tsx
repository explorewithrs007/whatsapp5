  ...
  const columns: DataTableColumn<CannedReplyRecord>[] = [
    ...
    { key: "status", header: "Status", cell: (row) => <StatusBadge status={row.status} /> },
    {
      ...
      cell: (row) => (
        <TableActions
          actions={[
            { icon: Pencil, label: "Edit reply", onClick: () => openEditDialog(row) },
            {
              icon: Power,
    ...
    <div className="flex w-full flex-col gap-4 pb-24">
      <SectionCard>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-semibold text-foreground">Predefined Responses</h2>
            <p className="mt-1 text-sm text-muted-foreground">Manage reusable reply shortcuts.</p>
          </div>
...
function CannedReplyDialog({
  ...
  return (
    <StandardDialog
      ...
      footerRight={<Button onClick={handleSave}>{isEditing ? "Save Changes" : "Save Reply"}</Button>}
