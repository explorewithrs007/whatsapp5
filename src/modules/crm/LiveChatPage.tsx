...
function LastMessageCell({ conversation }: { conversation: LiveChatConversation }) {
  ...
  return (
    <Tooltip align="start" label={conversation.lastMessageTooltip || "No message yet"}>
      <span className="flex h-6 min-w-0 max-w-full items-center gap-2 overflow-hidden text-sm text-foreground">
              ...
              <div>
                ...
                <div className="mt-4 space-y-3">
                  {notes.length ? (
                    ...
                    <p className="rounded-xl bg-slate-50/70 p-4 text-sm text-muted-foreground">No internal notes yet.</p>
                  )}
                </div>
              </div>
            </div>
