  ...
  const columns: DataTableColumn<LinkAliasRecord>[] = [
    ...
    { key: "conversations", header: "Clicks / Conversations", cell: (row) => row.conversations },
    {
      ...
      cell: (row) => (
        <TableActions
          actions={[
            { icon: Eye, label: "Preview link", onClick: () => setPreviewLink(row) },
            { icon: Pencil, label: "Edit link", onClick: () => openEdit(row) },
            { destructive: true, icon: Trash2, label: "Delete link", onClick: () => setDeleteLink(row) },
          ]}
        />
      ),
    },
...
function LinkFormDialog({
  ...
  return (
    <StandardDialog
      ...
      footerRight={<Button onClick={onSave}>{title === "Create Link" ? "Create Link" : "Save Link"}</Button>}
